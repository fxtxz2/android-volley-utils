...
public class MultipartFileRequest<T> extends Request<T> {
	 ...
	 */
	public MultipartFileRequest(int method, String url, Class<T> clazz,
		...
		for (Map.Entry<String, Object> entry : params.entrySet()) {
			...
			if (object instanceof File){
				entity.addStringFilePart(entry.getKey(), (File) object);
			} else if (object instanceof String){
				entity.addStringPart(entry.getKey(), (String) object);
			}
		}
	}
	...
	public byte[] getBody() throws AuthFailureError {
		...
		try {
			entity.writeTo(bos);
			return bos.toByteArray();// TODO OOM 容易发生内存溢出
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (bos != null) {
